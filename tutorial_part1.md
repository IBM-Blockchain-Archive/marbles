...
(2) Deploy the app on my local machine [manually](#network) connecting to a Bluemix BlockChain network 

`OR`

(3) Deploy the app on my machine [locally](#local) connecting to a [locally deployed](https://github.com/hyperledger/fabric/blob/master/docs/dev-setup/devnet-setup.md) BlockChain network 
...
This section explains the simplest network setup to make the Marbles app and the BlockChain nodes run locally altogether. Further advanced setups are suggested [here](#further)
...
2. Follow [this instruction](https://github.com/openblockchain/obc-docs/blob/master/dev-setup/devnet-setup.md) to setup 2 Validating peers (without using security, i.e. not starting a CA).
...
5. Remove the lines of code that stores the ChainCode summary file (it causes errors and it is not necessary):
...
- After deploying OBC one can try to setup the local network using Hyperledger following [this changes](https://github.com/IBM-Blockchain/marbles/issues/6) and adjusting the commands used for the development network setup (if you succeeded using HyperLedger please do document it here). 
...
- Try using another consensus plugin: the consensus to be used is defined in the VP configuration file (see Using Consensus Plugin section in the development setup instruction).
